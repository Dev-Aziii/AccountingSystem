...
- `api/superadmin/security-events` exposes only `AUTH-*` tenant security events across the platform.
...
- Open `/admin/audit-logs` and confirm only current-tenant activity is visible.
